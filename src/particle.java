...
public class particle {
	...
	void updateAcceleration(List<particle> particleList, int skip) {
		...
		for (int i = 0; i < len; i++) {
			...
			double changeW = (v - particle.v)/dist(this,particle)*1/SPHPoiseuille.H*WGrad(dist(this, particle));
			double viscosityTerm = SPHPoiseuille.PARTICLE_MASS*(100d*(y-particle.y)*(v-particle.v))/((density+particle.density)*dist(this,particle)); 
